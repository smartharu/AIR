...
class UNetResHA(Model):
    def __init__(self, in_nc: int = 3, out_nc: int = 3, nc: Sequence[int] | None = None,
                 nb: Sequence[int] | None = None, bias=False,upsample:str = "pixelshuffle",downsample:str="strideconv"):
        super(UNetResHA, self).__init__()
        ...
        self.down1 = B.sequential(
            B.ResidualHybridAttentionGroup(nc[0], 3, 1, 1, bias, self.reduction, nb[0]),
            B.DownSample(2, self.downsample, 3, 1, nc[0], nc[1], False))

        self.down2 = B.sequential(
            B.ResidualHybridAttentionGroup(nc[1], 3, 1, 1, bias, self.reduction, nb[1]),
            B.DownSample(2, self.downsample, 3, 1, nc[1], nc[2], False))
        self.down3 = B.sequential(
            B.ResidualHybridAttentionGroup(nc[2], 3, 1, 1, bias, self.reduction, nb[2]),
            B.DownSample(2, self.downsample, 3, 1, nc[2], nc[3], False))

        self.body = B.sequential(
            B.ResidualHybridAttentionGroup(nc[3], 3, 1, 1, bias, self.reduction, nb[3]),)
...
if __name__ == "__main__":
    ...
    #net = UNetResHA(3, 3, [32, 64, 128, 256], [4, 4, 4, 4], True)
