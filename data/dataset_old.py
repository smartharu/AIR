...
class RandomBlend:
    ...
    def __call__(self, x: torch.Tensor) -> torch.Tensor:
        ...
        alpha = random.uniform(0.91,0.991)
        if random.uniform(0,1)<self.color_prob:
            if random.uniform(0,1) < 0.5:
                if random.uniform(0,1)<0.8:
                    gray = random.randint(0,16)
                else:
                    gray = random.randint(235, 255)
            else:
                gray = random.randint(16, 235)
...
if __name__ == "__main__":
    ...
    #m = RandomRescale3(prob=0.8, scale=(0.375, 1.0), anisotropic_p=0.3, anisotrpoic_scale=0.25)
    #m = RandomRescale(prob=0.8, scale=(0.375, 1.0), anisotropic_p=0.3, anisotrpoic_scale=0.25)
    t = time.time()
    for _ in range(100):
        res = transform_IR(x)
